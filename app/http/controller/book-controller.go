...
func (c *bookController) Insert(context *gin.Context) {
    var bookCreateRequest request.BookCreateRequest
    errRequest := context.ShouldBind(&bookCreateRequest)
    if errRequest != nil {
        ...
        if err == nil {
            bookCreateRequest.UserID = convertedUserID
        }
        result := c.bookService.Insert(bookCreateRequest)
...
func (c *bookController) Update(context *gin.Context) {
    var bookUpdateRequest request.BookUpdateRequest
    errRequest := context.ShouldBind(&bookUpdateRequest)
    if errRequest != nil {
        res := helper.BuildErrorResponse("Failed to process request", errRequest.Error(), helper.EmptyObj{})
        context.JSON(http.StatusBadRequest, res)
        return
    ...
    if c.bookService.IsAllowedToEdit(userID, bookUpdateRequest.ID) {
        id, errID := strconv.ParseUint(userID, 10, 64)
        if errID == nil {
            bookUpdateRequest.UserID = id
        }
        result := c.bookService.Update(bookUpdateRequest)
        response := helper.BuildResponse(true, "OK", result)
